...
def load_set(path):
    ...
    if path == "3":
        ...
        while True:
            ...
            if arguments == "del":
                delKey = input("Enter the key which you want to remove: ")
                if delKey in definitions:
                    del definitions[delKey]
                    print("succesfully removed '%s' from %s" % (delKey, name) )
                else:
                    print("%s isn't a key in the dictionary: '%s'" % (delKey, name))
...
def test_for_term(definitions, listofkeys):
    ...
    while x > 0:
        ...
        if start == 'help':
            ...
            while n > 0:
                ...
                if start == 'start':
                    break
        if start != 'start' and start != 'help' and start != "list terms":
    ...
    while start == 'start':
        ...
        if getcorrect(userguess, term) is True:
            ...
            print('')
            next = input("")
            if(next == "i was right" or next == "override"):
                correct += 1
                listofkeys.remove(term)
            else:
                incorrect += 1
